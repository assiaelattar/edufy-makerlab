...
export const generateCompletionCertificate = (student: Student, enrollment: Enrollment, settings: AppSettings, options: { modules: string[], academyName: string, logoUrl: string, issueDate?: string }) => {
  const win = window.open('', '_blank');
  if (!win) return;

  const academyName = options.academyName || 'Makerlab Academy';
  ...
  const modulesHtml = options.modules.length > 0 
    ? `
      <div style="margin-top: 30px; margin-bottom: 30px;">
        <h3 style="font-size: 16px; color: var(--primary-black); margin-bottom: 10px;">Modules validés :</h3>
        <ul style="list-style-type: disc; padding-left: 20px; color: #334155; line-height: 1.8;">
          ${options.modules.map(m => `<li>${m}</li>`).join('')}
        </ul>
      </div>
      `
    : '';
  ...
  const html = `
      ...
      <head>
        <meta charset="UTF-8">
        <title>Attestation de réussite - ${student.name}</title>
        <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&family=Playfair+Display:wght@700&display=swap" rel="stylesheet">
        <script src="https://cdn.tailwindcss.com"></script>
        <style>
          @import url('https://fonts.googleapis.com/css2?family=Roboto:wght@300;400;700&display=swap');
      ...
      <body>
        ...
        <div class="certificate-container">
          ...
          <div class="content">
            ...
            <p>
              A suivi avec assiduité et validé avec succès le programme de formation suivant :
            </p>

            <div class="program-box">
               <div class="program-value">${enrollment.programName}</div>
            </div>

            ${modulesHtml}

            <p>
              Cette attestation est délivrée à l'intéressé(e) pour servir et valoir ce que de droit, en reconnaissance des compétences acquises et des efforts fournis tout au long du programme.
            </p>
          </div>
