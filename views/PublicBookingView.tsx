        ...
        <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 max-w-md w-full text-center animate-in zoom-in duration-300">
                ...
                <p className="text-slate-400 mb-6">
                    We've confirmed <strong>{bookingForm.kidName}'s</strong> spot for <br/>
                        ...
                        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 md:p-8">
                            ...
                            <form onSubmit={handleBooking} className="space-y-5">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    <div>
                                        <label className="block text-xs font-medium text-slate-400 mb-1.5 uppercase tracking-wider">Parent Name</label>
                                        <input required className="w-full p-3 bg-slate-950 border border-slate-800 rounded-lg text-white focus:border-blue-500 outline-none transition-colors" value={bookingForm.parentName} onChange={e => setBookingForm({...bookingForm, parentName: e.target.value})} placeholder="John Doe" />
                                    </div>
                                    <div>
                                ...
                                <div className="grid grid-cols-3 gap-5">
                                    <div className="col-span-2">
                                        <label className="block text-xs font-medium text-slate-400 mb-1.5 uppercase tracking-wider">Child Name</label>
                                        <input required className="w-full p-3 bg-slate-950 border border-slate-800 rounded-lg text-white focus:border-blue-500 outline-none transition-colors" value={bookingForm.kidName} onChange={e => setBookingForm({...bookingForm, kidName: e.target.value})} placeholder="Child's Name" />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-medium text-slate-400 mb-1.5 uppercase tracking-wider">Age</label>
                                        <input required type="number" className="w-full p-3 bg-slate-950 border border-slate-800 rounded-lg text-white focus:border-blue-500 outline-none transition-colors" value={bookingForm.kidAge} onChange={e => setBookingForm({...bookingForm, kidAge: e.target.value})} placeholder="e.g. 8" />
                                    </div>
                                </div>
                                <div>
                                    <label className="block text-xs font-medium text-slate-400 mb-1.5 uppercase tracking-wider">Interests / Notes (Optional)</label>
                                    <textarea className="w-full p-3 bg-slate-950 border border-slate-800 rounded-lg text-white focus:border-blue-500 outline-none transition-colors h-24 resize-none" value={bookingForm.kidInterests} onChange={e => setBookingForm({...bookingForm, kidInterests: e.target.value})} placeholder="e.g. Loves Lego, Coding, Minecraft..." />
                                </div>
