...
import { Hammer, Plus, ExternalLink, Trash2, Search, Cpu, Box, CheckSquare, User, RotateCcw, Database } from 'lucide-react';
...
import { MOCK_TOOLS, MOCK_ASSETS } from '../utils/mockData';
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 overflow-y-auto">
                    ...
                    {filteredTools.length === 0 && (
                        <div className="col-span-full flex flex-col items-center justify-center py-20 text-slate-400 bg-white rounded-[2.5rem] border border-slate-100 border-dashed">
                            ...
                            {can('toolkit.manage') && toolLinks.length === 0 && (
                                <button
                                ...
                                >
                                    <Database size={16} /> {isSeeding ? 'Loading...' : 'Load Sample Tools'}
                                </button>
                            )}
                        </div>
                    )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 overflow-y-auto">
                            ...
                            <div key={asset.id} className="bg-white border border-slate-100 rounded-[2rem] p-6 hover:shadow-xl hover:-translate-y-1 transition-all group relative shadow-sm">
                                ...
                                <div className="mt-auto pt-4 border-t border-slate-50">
                                    {isAvailable ? (
                                        ...
                                        ) : <div className="text-center text-xs text-emerald-600 font-bold bg-emerald-50 py-2 rounded-lg">Available for Checkout</div>
                                    ) : (
                                        can('toolkit.manage') ? (
                                        ...
                                        ) : <div className="text-center text-xs text-amber-600 font-bold bg-amber-50 py-2 rounded-lg">Currently In Use</div>
                                    )}
                                </div>
                            </div>
                        )
                    ...
                    {filteredAssets.length === 0 && (
                        <div className="col-span-full flex flex-col items-center justify-center py-20 text-slate-400 bg-white rounded-[2.5rem] border border-slate-100 border-dashed">
                            ...
                            {can('toolkit.manage') && assets.length === 0 && (
                                <button
                                    onClick={handleSeedAssets}
                                    disabled={isSeeding}
                                    className="flex items-center gap-2 px-6 py-3 bg-[#2D2B6B] hover:bg-indigo-800 text-white rounded-xl text-sm font-bold transition-colors shadow-lg shadow-indigo-900/20"
                                >
                                    <Database size={16} /> {isSeeding ? 'Loading...' : 'Load Sample Inventory'}
                                </button>
                            )}
                        </div>
                    )}
