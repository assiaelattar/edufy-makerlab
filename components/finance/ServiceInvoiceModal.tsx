...
import { issueFinanceInvoice, normalizeLines, updateFinanceInvoice, type IssueFinanceInvoiceInput } from '../../services/financeDocuments';
...
interface Props {
  ...
  editingInvoice?: FinanceDocument;
  onClose: () => void;
  onSaved: (invoice: FinanceDocument) => void;
}
const blankCustomer = (): FinanceCustomerSnapshot => ({ type: 'company', name: '', address: '', contactName: '', email: '', phone: '', ice: '', rc: '' });
type EditableLine = IssueFinanceInvoiceInput['lines'][number] & { key: string };
export function ServiceInvoiceModal({ organizationId, services, documents, settings, initialService, editingInvoice, onClose, onSaved }: Props) {
  const isEditing = Boolean(editingInvoice);
  const [customer, setCustomer] = useState<FinanceCustomerSnapshot>(() => editingInvoice ? { ...editingInvoice.customer } : blankCustomer());
  const [clientSearch, setClientSearch] = useState('');
  const [serviceSearch, setServiceSearch] = useState('');
  const [lines, setLines] = useState<EditableLine[]>(() => editingInvoice
    ? editingInvoice.lines.map((line, index) => ({ ...line, key: `saved-${line.id || index}` }))
    : initialService ? [{ ...snapshotService(initialService, organizationId), key: crypto.randomUUID() }] : []);
  const [dates, setDates] = useState(() => {
    const date = new Date().toISOString().slice(0, 10);
    return editingInvoice
      ? { issueDate: editingInvoice.issueDate, dueDate: editingInvoice.dueDate || editingInvoice.issueDate, serviceDate: editingInvoice.serviceDate || editingInvoice.issueDate }
  ...
  const currency = editingInvoice?.currency || settings.currency || 'MAD';
  ...
  const cleanCustomer = () => ({ ...customer, name: customer.name.trim(), ice: customer.type === 'company' ? customer.ice : undefined, rc: customer.type === 'company' ? customer.rc : undefined });
  const editableLines = () => lines.map(({ key, ...line }) => ({ ...line, unitKind: line.unitKind || 'package' as const, calculationMode: line.calculationMode || 'manual' as const }));
  const buildIssueInput = () => ({ organizationId, sequenceType: 'service' as const, ...dates, currency, customer: cleanCustomer(), participants: [], idempotencyKey: idempotencyKey.current, lines: editableLines() });
  const buildUpdateInput = () => ({ organizationId, invoiceId: editingInvoice!.id, expectedRevision: Math.max(0, Number(editingInvoice!.revision) || 0), ...dates, customer: cleanCustomer(), lines: editableLines() });
  const validate = () => {
  ...
  const preview = () => {
    try {
      validate(); const input = isEditing ? buildUpdateInput() : buildIssueInput();
      const record: FinanceDocument = editingInvoice
        ? { ...editingInvoice, ...dates, customer: cleanCustomer(), lines: normalizeLines(input.lines), subtotal, taxAmount, total }
        : { ...buildIssueInput(), id: 'preview', kind: 'invoice', status: 'issued', number: 'APERÇU — NON ÉMISE', sequenceYear: Number(dates.issueDate.slice(0, 4)), sequenceNumber: 0, lines: normalizeLines(input.lines), subtotal, taxAmount, total };
      if (!generateFinanceDocument(record, settings, { preview: !isEditing, editPreview: isEditing })) setError('Autorisez les fenêtres contextuelles pour ouvrir l’aperçu.');
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const invoice = isEditing
        ? await updateFinanceInvoice(db, buildUpdateInput())
        : await issueFinanceInvoice(db, buildIssueInput());
      setIssued(invoice); onSaved(invoice);
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Facture non enregistrée. Réessayez.'); }
    finally { saving.current = false; setBusy(false); }
  };
  const modalTitle = issued
    ? `Facture ${issued.number} ${isEditing ? 'mise à jour' : 'émise'}`
    : isEditing ? `Modifier la facture ${editingInvoice!.number}` : 'Nouvelle facture de services';
  return <Modal isOpen onClose={() => !busy && onClose()} title={modalTitle} size="xl">
    <div className="service-workspace">{issued ? <><p role="status">La facture {issued.number} est {isEditing ? 'mise à jour dans' : 'enregistrée dans'} l’historique.</p><div className="service-totals"><span>{issued.customer.name}</span><strong>{money(issued.total)} TTC</strong></div><div className="service-actions"><button onClick={onClose}>Fermer</button><button className="primary" onClick={() => { if (!generateFinanceDocument(issued, settings)) setError('Autorisez les fenêtres contextuelles pour ouvrir le PDF.'); }}>Imprimer / PDF</button></div></> : <form onSubmit={submit} className="service-form">
      {isEditing && <div className="service-edit-notice"><strong>N° {editingInvoice!.number} conservé</strong><span>Version {Math.max(0, Number(editingInvoice!.revision) || 0) + 1} · les avoirs et les factures créditées restent verrouillés.</span></div>}
      <div className="service-steps"><strong>{review ? '3 · Vérification et sauvegarde' : '1 · Client'}</strong><span>2 · Lignes et détails</span>{!review && <span>3 · Aperçu et sauvegarde</span>}</div>
      {review ? <>
        ...
        <p className="service-note">{isEditing ? `La facture conservera le numéro ${editingInvoice!.number}. Cette sauvegarde créera la version ${Math.max(0, Number(editingInvoice!.revision) || 0) + 1}.` : 'À l’émission, le numéro définitif est attribué. La facture pourra ensuite être corrigée depuis l’historique tant qu’elle n’est pas créditée.'}</p>
